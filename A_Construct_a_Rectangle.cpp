...
public:
    void solve() {
       ...
       //freopen("output.txt", "w", stdout);
        ...
        //if two sides are already equal -> just break the third side
        // to break the third side equally -> we should have an even length side
        if(a==b){
            if(c%2==0){cout<<"YES";return;}
            flag=true;
        }
        if(a==c){
            if(b%2==0){cout<<"YES";return;}
            flag=true;
        }
        if(b==c){
            if(a%2==0){cout<<"YES";return;}
            flag=true;
        }
        if(flag){
            cout<<"NO";
        ...
        //no two sides are equal -> any two sides will become adjacent sides of the rectangle
        //so third side should be equal to the sum of the two sides -> cause the other two sides will be made out of this one only
