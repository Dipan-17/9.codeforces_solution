...
class Solution {
public:
    void genSubs(string s, vector<string>&subseq, string current, int index) {
        if(index==s.length()) {
            subseq.push_back(current);
            return;
        }
        genSubs(s,subseq,current,index+1);
        genSubs(s,subseq,current+s[index],index+1);
    }
    ...
    void solve() {
        ...
        string current="";
        genSubs(s,subseq,current,0); //generate subsequences

        //only palindrome
        vector<string>palin;
        for(auto iter:subseq){
