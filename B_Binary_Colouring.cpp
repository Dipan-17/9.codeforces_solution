...
public:
    void solve() {
       ...
       //freopen("output.txt", "w", stdout);
        ...
        //in binary: sum upto n-1 = pow(2,n)-1
        //eg: 001110 = 28, 000001=32, where 32=28+4 = 32-smallest previous set bit

        ff(i,0,32){
            if(v[i]==0)continue;

            //check if consecutive ones and set them to 0
            int j=i+1;
            while (v[j])
            {
                v[j]=0;
