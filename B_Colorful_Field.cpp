...
public:
    void solve() {
       ...
       //freopen("output.txt", "w", stdout);
        ...
        //k lines giving coordinates of waste land
        set<ll>waste;
        vl w;
        while(k--){
            ll a,b;cin>>a>>b;
            waste.insert((a-1)*m+(b-1));
            w.pb((a-1)*m+(b-1));
        }
        sv(w);
        //t lines query
        while(t--){
            int a,b;cin>>a>>b;
            ll c=(a-1)*m+(b-1);
            if(waste.find(c)!=waste.end()){
                cout<<"Waste\n";
            }else{
                //we need to calcualte the number of waste before the current
                ll j=0;
                while(j<w.size() && w[j]<c)j++;
                ll filled=c-j;
                filled%=3;
                cout<<crop[filled]<<endl;
            }
        }
    }
};
