...
public:
    void solve() {
       ...
       //freopen("output.txt", "w", stdout);
        ...
        while(q--){
            ll li,ri;cin>>li>>ri;
            string sub = s.substr(li - 1, ri - li + 1);

            

            if(sub.size()<2 || sub.size()==s.size()){cn;nl;continue;}
            //we need a substring in s that is equal to sub but is not continuous


            //for all except one element of sub -> take from the start of the sequence
            //from the last taken index -> leave one index
            //try to find the last element of sub

            int i=0,j=0;
            while (i<s.size() && s[i]!=sub[j])i++;
            
            

            bool iscon=true;
            
            while(i<s.size() && j<sub.size()-1){
                if(s[i]==sub[j]){i++;j++;}
                else {i++;iscon=false;}
            }
