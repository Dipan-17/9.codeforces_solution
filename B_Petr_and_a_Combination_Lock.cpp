...
public:
    ...
    void generateSubset(vl v, vl& temp, int index, bool& flag) {
    if (index == v.size()) {
        ll s = accumulate(temp.begin(), temp.end(), 0);
        if (s % 360 == 0)flag = true;
        
        return;
    }

    temp.push_back((-1) * v[index]);
    generateSubset(v, temp, index + 1, flag);
    temp.pop_back();
    ...
    void solve() {
        bool flag = false;  // Local flag for each test case
        ll n;
        cin >> n;
        vl v(n);
        readvector(v);
        vl temp;
        generateSubset(v, temp, 0, flag);
        if(!flag) cout << "NO";
        else cout << "YES";
    }
};
