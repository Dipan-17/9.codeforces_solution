...
public:
    void solve() {
       ...
       //freopen("output.txt", "w", stdout);
        ...
        ll smoll=mp.begin()->first;
        while(mp.size()){
            mp[smoll]--;
            if(mp[smoll]==0)mp.erase(smoll);
            
            ff(i,1,n){
                ll curr=smoll+i*d;
                if(mp.find(curr)==mp.end()){cout<<"NO";return;}
                else{
                    mp[curr]--;
            ...
            if(mp.size()==0){cout<<"YES";return;}

            ll next=smoll+c;
            if(mp.find(next)==mp.end()){cout<<"NO";return;}
            else{
                smoll=next;
            }
