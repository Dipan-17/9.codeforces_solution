...
public:
    void solve() {
       ...
       //freopen("output.txt", "w", stdout);
        ...
        if(zc==oc){cout<<0;return;}
        //if zero count and one count are not equal
        ll count=s.size(),ans=count;
        for(auto iter:s){
            //swap
            if(iter=='0')oc--;
            else zc--;
            ...
            count--;
            if(oc<0 || zc<0)break;
            ans=count;//if breaks this line is never touched -> cause it was an invalidd
        }
        cout<<ans;
    }
};
