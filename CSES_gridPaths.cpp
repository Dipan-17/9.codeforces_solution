...
public:
    ll n;
    ll solver(vector<vector<char>>&grid,int row,int col,vector<vector<ll>>&dp){
        if(row>=n || col>=n )return 0;
        if(row==n-1 && col==n-1 && grid[row][col]=='.')return 1;
        if(grid[row][col]=='*')return 0;

        if(dp[row][col]!=-1)return dp[row][col];

        ll right=solver(grid,row,col+1,dp)%MOD;
        ll down=solver(grid,row+1,col,dp)%MOD;

        ll ans=(right+down)%MOD;
    ...
    void solve() {
       ...
       //freopen("output.txt", "w", stdout);
        ...
        ll ans=0;
        if(grid[0][0]!='*')ans=solver(grid,0,0,dp);
        cout<<ans;
    }
};
