...
public:
    void solve() {
       ...
       ll ans=0;
        ...
        ff(i,0,3){//i=0 -> 0th element differs and so on
            map<pair<ll,ll>,vl>mp;

            ff(j,0,n-2){
                //creating the map of all possible triplets
                if(i==0){
                    mp[{a[j+1],a[j+2]}].pb(a[j]);//first element is different in the triplet
                }else if(i==1){
                    mp[{a[j],a[j+2]}].pb(a[j+1]);
                }else{
                    mp[{a[j],a[j+1]}].pb(a[j+2]);
                }
            }

            for(auto x:mp){
                //sort all the possible numbers for each triplet
                sort(all(x.second));
                vl t;
                ll c=1;
                ff(k,1,x.second.size()){
                    if(x.second[k]==x.second[k-1])c++;//duplicates
                    else{
                        t.pb(c);
                        c=1;
                    }
                ...
                t.pb(c);
                if(t.size()<2)continue;//no beautiful triplets
                ll com=accumulate(all(t),0ll);
                com=com*(com-1)/2; //total number of different pairs = n(n-1)/2
                ans+=com;
                for(auto x:t){
                    ans-=(x*(x-1)/2);
                }
