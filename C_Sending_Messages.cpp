...
public:
    void solve() {
       ...
       //freopen("output.txt", "w", stdout);
        ...
        vl v;
        v.pb(0);
        ff(i,0,n){ll x;cin>>x;v.pb(x);}
        
        //if m(i+1)-m(i)>a -> turn off ->cost = b
        // else keep on -> cost = a*(time)
        
        ll p=1;
        while(f>0 && p<=n){//cause 0 pushed back
            ll onCost=(v[p]-v[p-1])*a;
            ll offCost=b;
            f-=min(offCost,onCost);
            p++;
        }
