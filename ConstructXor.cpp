...
class Solution {
public:
    vector<int> generateArray(int n, int s) {
    if (n == 0 || s < 0) {
        return {-1}; // Handle invalid cases
    }

    // Check if sum is divisible by n (all elements must be even)
    if (s % n != 0) {
        return {-1};
    }

    // Initialize array with elements of 2
    vector<int> arr(n, 2);

    // If sum is even, distribute remaining even numbers
    if (s % 2 == 0) {
        int remaining = s / n - 1;
    ...
    // If sum is odd, check for a single odd number
    else {
        if (n == 1) {
            return {-1}; // Odd sum with n=1 is impossible
        }
    ...
    void solve() {
       ...
       //freopen("output.txt", "w", stdout);
        ll n,s;cin>>n>>s;
        vector<int>result=generateArray(n,s);
